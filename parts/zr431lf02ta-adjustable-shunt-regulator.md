---
type: ic
category: Voltage Regulators
subcategory: Adjustable Shunt Regulators
part_number: ZR431LF02TA
description: Precision adjustable shunt voltage regulator, SOT-23 package
manufacturer: Diodes Incorporated
package: SOT-23 (3-pin)
voltage: 2.5V to 36V operating range
current: Up to 100mA output current
interface: Adjustable via external resistors
location: SMT-Parts-Book-1/Page-5/Slot-1
quantity: 10
status: available
product_url: https://www.digikey.com/en/products/detail/diodes-incorporated/ZR431LF02TA/280652
datasheet: https://www.diodes.com/assets/Uploads/ZR431.pdf
tags: [smt, voltage-regulator, shunt-regulator, adjustable, sot-23, diodes-incorporated, power-management, reference-voltage]
date_added: 2025-11-16
price: $0.50-1.00 each
---

## Description

This is a high-performance adjustable shunt regulator suitable for voltage regulation, voltage reference generation, and switching power supply feedback control applications. The device offers low dynamic impedance and high stability.

## Specifications

- **Type:** Adjustable Shunt Regulator
- **Package:** SOT-23 (3-pin)
- **Reference Voltage:** Typically 2.5V (adjustable via external resistors)
- **Operating Voltage Range:** 2.5V to 36V
- **Output Current:** Up to 100mA
- **Temperature Range:** -40°C to +125°C (industrial grade)
- **Features:** Low dynamic impedance, high stability, low noise

## Applications

- Voltage regulation circuits
- Voltage reference generation
- Switching power supply feedback control
- Precision current sources
- Overvoltage protection circuits

## Storage

Store in cool, dry location. Keep in protective sleeve to prevent damage.

